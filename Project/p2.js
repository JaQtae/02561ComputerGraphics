...
window.onload = async function init()
{
    ...
    const ext1 = gl.getExtension("WEBGL_debug_renderer_info");
    if (ext1) {
    console.log("WebGL Debug Info:", gl.getParameter(ext1.UNMASKED_RENDERER_WEBGL));
    }
    ...
    texwidth = 512.0; // 2048
    texheight = 512.0;
    fbo = initFramebufferObject(gl, texwidth, texheight);
    ...
    function tick(){     
        ...
        L_eye = vec3(lightPosition[0], lightPosition[1], lightPosition[2]);
        L_at = vec3(0.0, -1.0, -3.0);

        v_light = lookAt(L_eye, L_at, up);
        p_light = perspective(fovy, canvas.width / canvas.height, near, far);
...
function draw_ground(alpha) {
    ...
    // Shadow texture to ground shader and binding
    ...
    gl.uniformMatrix4fv(vLoc_light_GROUND, false, flatten(v_light));
    gl.uniformMatrix4fv(pLoc_GROUND, false, flatten(p_light));
...
function draw_shadow() {
    ...
    gl.uniformMatrix4fv(vLoc_SHADOW, false, flatten(v_light));
    gl.uniformMatrix4fv(pLoc_SHADOW, false, flatten(p_light));

    // Drawing teapot
    gl.enable(gl.CULL_FACE); // To make shadows appear regardless of light position
    gl.cullFace(gl.BACK);
    gl.drawElements(gl.TRIANGLES, g_drawingInfo.indices.length, gl.UNSIGNED_INT, 0);
    // End of framebuffer
