...
window.onload = async function init() {
  ...
  function tick() {
    ...
    if (jump) {
        if (y_coord.toFixed(2) == -1.8) { Wy = 0.01; }
  ...
  function render() {
        ...
        planar_coefficients = get_plane_coefficients(mat4(), v, vec4(0.0, -1.0, 0.0, 1.0), vec4(0.0, -1.0, 0.0, 1.0));
        var P_reflect = modifyProjectionMatrix(planar_coefficients, p);

        draw_object(reflect_coeff = 1.0, p_ = P_reflect);
        ...
        gl.clear(gl.DEPTH_BUFFER_BIT); // Clear out so ground is behind teapot
...
function draw_object(reflect_coeff, p_ = p) {
    gl.useProgram(program_OBJECT);
    ...
    gl.uniformMatrix4fv(pLoc_OBJECT, false, flatten(p_));
...
function modifyProjectionMatrix(clipplane, projection) {
    ...
    var q = vec4((Math.sign(clipplane[0]) + projection[0][2])/projection[0][0], 
                ...
                -1.0, (1.0 + projection[2][2])/projection[2][3]);
...
function get_plane_coefficients(M, V, planar_point, planar_normal) {
    // https://forum.gamemaker.io/index.php?threads/projection-matrix-with-oblique-near-clip-plane.60537/

    // worldspace point and normal - Ground-plane
    var W_point = mult(M, planar_point);
    var W_normal = mult(M, planar_normal);

    // Camera-space position.
    var Px, Py, Pz;

    Px = V[0][0] * W_point[0] + V[1][0] * W_point[1] + V[2][0] * W_point[2] + V[3][0];
    Py = V[0][1] * W_point[0] + V[1][1] * W_point[1] + V[2][1] * W_point[2] + V[3][1];
    Pz = V[0][2] * W_point[0] + V[1][2] * W_point[1] + V[2][2] * W_point[2] + V[3][2];

    // Camera-space 4D reflection plane!
    var Cx, Cy, Cz, Cw;
    Cx = V[0][0] * W_normal[0] + V[1][0] * W_normal[1] + V[2][0] * W_normal[2];
    Cy = V[0][1] * W_normal[0] + V[1][1] * W_normal[1] + V[2][1] * W_normal[2];
    Cz = V[0][2] * W_normal[0] + V[1][2] * W_normal[1] + V[2][2]* W_normal[2];
